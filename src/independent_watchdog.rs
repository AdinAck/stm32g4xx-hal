...
impl IndependentWatchdog {
    ...
    pub fn start_windowed<T: Into<MicroSecond>>(&mut self, min_window_time: T, max_window_time: T) {
        ...
        self.iwdg.pr().write(|w| {
            w.pr().variant(*prescaler);
        });
        ...
        self.iwdg.winr().write(|w| unsafe {
            w.win().bits(Self::MAX_COUNTER_VALUE as u16);
        });
        ...
        self.iwdg.rlr().write(|w| unsafe {
            w.rl().bits(reload_value as u16);
        });
        ...
        self.iwdg.winr().write(|w| unsafe {
            w.win().bits((reload_value - window_value) as u16);
        });
