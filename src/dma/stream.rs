...
macro_rules! dma_stream {
    ($(($name:ident, $number:expr,
        dmamux => $dma1_cXcr:expr, $dma2_cXcr:expr, )
    ),+$(,)*) => {
        $(
            impl<I: Instance> Stream for $name<I> {
                ...
                fn clear_interrupts(&mut self) {
                    ...
                    dma.ifcr().write(|w| {
                        w
                            .ctcif($number).set_bit() //Clear transfer complete interrupt flag
                            .chtif($number).set_bit() //Clear half transfer interrupt flag
                            .cteif($number).set_bit() //Clear transfer error interrupt flag
                            .cgif($number).set_bit(); //Clear global interrupt flag
                    });
                    let _ = dma.isr().read();
                ...
                fn clear_transfer_complete_flag(&mut self) {
                    ...
                    // that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ifcr().write(|w| { w.ctcif($number).set_bit(); });
                }
                ...
                fn clear_transfer_error_interrupt(&mut self) {
                    ...
                    // that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ifcr().write(|w| { w.cteif($number).set_bit(); });
                    let _ = dma.isr().read();
                    let _ = dma.isr().read(); // Delay 2 peripheral clocks
                }
                ...
                fn get_transfer_complete_flag() -> bool {
                    //NOTE(unsafe) Atomic read with no side effects
                    let dma = unsafe { &*I::ptr() };
                    dma.isr().read().tcif($number).bit_is_set()
                }
                ...
                fn get_transfer_error_flag() -> bool {
                    //NOTE(unsafe) Atomic read with no side effects
                    let dma = unsafe { &*I::ptr() };
                    dma.isr().read().teif($number).bit_is_set()
                }
                ...
                unsafe fn enable(&mut self) {
                    ...
                    let dma = &*I::ptr();
                    dma.ch($number).cr().modify(|_, w| {w.en().set_bit();});
                }
                ...
                fn is_enabled() -> bool {
                    //NOTE(unsafe) Atomic read with no side effects
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).cr().read().en().bit_is_set()
                }
                ...
                fn disable(&mut self) {
                    if Self::is_enabled() {
                        ...
                        dma.ch($number).cr().modify(|_, w| {w.en().clear_bit();});
                ...
                fn set_request_line(&mut self, request_line: u8) {
                    ...
                    unsafe {
                        if I::IS_DMA1 {
                            dmamux.ccr($dma1_cXcr)
                                .modify(|_, w| {w.dmareq_id().bits(request_line);});
                        } else {
                            dmamux.ccr($dma2_cXcr)
                                .modify(|_, w| {w.dmareq_id().bits(request_line);});
                        };
                    }
                }
                ...
                fn set_priority(&mut self, priority: config::Priority) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).cr().modify(|_, w| unsafe { w.pl().bits(priority.bits()); });
                }

                #[inline(always)]
                fn disable_interrupts(&mut self) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dmacr = &unsafe { &*I::ptr() }.ch($number).cr();
                    dmacr.modify(|_, w| {
                        w
                        .tcie().clear_bit()
                        .teie().clear_bit()
                ...
                fn get_half_transfer_flag() -> bool {
                    //NOTE(unsafe) Atomic read with no side effects
                    let dma = unsafe { &*I::ptr() };
                    dma.isr().read().htif($number).bit_is_set()
                }
                ...
                fn clear_half_transfer_interrupt(&mut self) {
                    ...
                    // that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ifcr().write(|w| { w.chtif($number).set_bit(); });
                    let _ = dma.isr().read();
                    let _ = dma.isr().read(); // Delay 2 peripheral clocks
                }
                ...
                unsafe fn set_peripheral_address(&mut self, value: u32) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = &*I::ptr();
                    dma.ch($number).par().write(|w| {w.pa().bits(value);});
                }
                ...
                unsafe fn set_memory_address(&mut self, value: u32) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = &*I::ptr();
                    dma.ch($number).mar().write(|w| {w.ma().bits(value);});
                }
                ...
                fn get_memory_address(&self) -> u32 {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).mar().read().ma().bits()
                }
                ...
                fn set_number_of_transfers(&mut self, value: u16) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).ndtr().write(|w| unsafe { w.ndt().bits(value); });
                }
                ...
                fn get_number_of_transfers() -> u16 {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).ndtr().read().ndt().bits()
                }
                #[inline(always)]
                unsafe fn set_memory_size(&mut self, size: u8) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = &*I::ptr();
                    dma.ch($number).cr().modify(|_, w| {w.msize().bits(size);});
                }
                ...
                unsafe fn set_peripheral_size(&mut self, size: u8) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = &*I::ptr();
                    dma.ch($number).cr().modify(|_, w| {w.psize().bits(size);});
                }
                ...
                fn set_memory_increment(&mut self, increment: bool) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).cr().modify(|_, w| {w.minc().bit(increment);});
                }
                ...
                fn set_peripheral_increment(&mut self, increment: bool) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).cr().modify(|_, w| {w.pinc().bit(increment);});
                }
                ...
                fn set_circular_buffer(&mut self, circular_buffer: bool) {
                    //NOTE(unsafe) We only access the registers that belongs to the StreamX
                    let dma = unsafe { &*I::ptr() };
                    dma.ch($number).cr().modify(|_, w| {w.circ().bit(circular_buffer);});
                }
            }
